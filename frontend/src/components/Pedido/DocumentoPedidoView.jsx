// ============================================================
// O PEDIDO DE VENDA — A FOLHA.
//
// UMA FOLHA SÓ PARA OS DOIS LADOS. O ERP (Pedido de Venda — Documento) e
// o portal do cliente (Baixar Pedido em PDF) desenham ESTE componente,
// com os mesmos dados. Antes o portal imprimia a própria tela de
// acompanhamento, e o cliente saía com um papel diferente do que o
// vendedor tinha na mão.
//
// Uma tela, dois documentos: o que se VÊ e o que se IMPRIME.
//
// Na tela ele segue o visual do sistema — escuro, com os blocos
// destacados. No papel ele vira preto e branco: o ERP é escuro, papel
// não é, e imprimir a tela escura gasta meio cartucho para sair
// ilegível. É a mesma marcação, com um `@media print` que troca o mundo
// inteiro para branco.
//
...
// assim que ela vai sair no papel.
// ============================================================
import { useEffect, useState } from 'react';
import {
  ArrowLeft, Printer, Download, Eye, Image as ImageIcon, Frame,
...
import { fmtBRL, fmtUn, fmtDate } from '@/pages/Vendedor/ui';
...
/**
 * @param p            o pedido, no formato de /area-vendedor/pedidos/:id
 * @param onVoltar     para onde a seta leva (ERP ou portal)
 * @param autoImprimir abre o diálogo de impressão assim que a folha existe
 */
export default function DocumentoPedidoView({ p, onVoltar, autoImprimir = false }) {
  ...
  // documento ainda carregando imprime uma página em branco.
  useEffect(() => {
    if (!p || !autoImprimir) return;
  ...
  // As colunas do meio saem dos itens DESTE pedido: um copo tradicional
  // não abre "Cor da boca" vazia no documento que vai para o cliente.
  const colunas = [];
    ...
    <>
      <style>{`
        /* ═══ O QUE VAI PARA O PAPEL ═══════════════════════════
           O ERP INTEIRO SAI DO CAMINHO: o navegador imprime a PÁGINA, e
           a página é o ERP com o documento dentro dele. Caem o menu e o
           cabeçalho, o overflow do shell (que cortava tudo o que passava
           da altura da tela) e a altura fixa de 100vh.
        */
        @media print {
          ...
          /* No portal não há shell: é o fundo escuro da página pública
             que sai do papel. */
          .doc-pagina-publica { background: #ffffff !important; padding: 0 !important; min-height: 0 !important; }
          ...
          /* ── COLORIDO ────────────────────────────────────────
             "print-color-adjust: exact" manda o navegador imprimir os
             fundos: sem ele, ele "economiza tinta" e devolve texto claro
             sobre papel branco — ilegível. */
      ...
      <div className="doc-chrome flex flex-wrap items-start justify-between gap-4 mb-4">
        <div className="flex items-start gap-3">
          <button onClick={onVoltar} className="btn-secondary mt-0.5">
            <ArrowLeft size={15} />
          </button>
          <div>
      ...
      <div className={`doc-modo-${modo} flex flex-col xl:flex-row gap-4 items-start ${comArte ? '' : 'doc-sem-arte'}`}>
          ...
          <Quadro titulo="Itens do Pedido" Icon={ShoppingCart} cor={CIANO} className="mb-4">
            <div className="overflow-x-auto -mx-1">
              <table className="w-full text-[11px]" style={{ borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {/* "Linha" é o tipo de tinta (PS) e anda logo depois da
                        cor da personalização, que é o campo que ela explica. */}
        ...
        {/* "Sem arte" tira a arte daqui também, e não só do papel: o
            botão é uma PRÉVIA do que vai sair. */}
        {comArte && (
          ...
          <div className="rounded-2xl p-4"
            ...
            {temArte ? (
              ...
              <p className="text-[11px]" style={{ color: 'rgba(255,255,255,0.5)' }}>
                Este pedido ainda não tem arte anexada.
              </p>
            )}
          </div>
        </aside>
      ...
      {perguntando && (
        <div className="doc-chrome fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-lg rounded-2xl p-5 sm:p-6" onClick={e => e.stopPropagation()}
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <EscolhaPdf Icon={Palette} cor={ROSA} onClick={() => imprimir('cor')}
                titulo="Colorido" nota="Igual a esta tela." />
            </div>
          </div>
        </div>
      )}
