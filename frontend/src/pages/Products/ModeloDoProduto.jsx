...
// ════════════════════════════════════════════════════════════
// O PREÇO QUE CAI COM A QUANTIDADE — QUE MUDOU DE ENDEREÇO.
//
// Ele se editava aqui, e aqui estava errado: o preço de TABELA sai da
// Formação de Preço e o desconto por volume saía desta janela, sem
// nenhuma das duas telas saber da outra. Duas portas para a mesma
// decisão é a decisão tomada duas vezes, com dois resultados.
//
// E a Formação de Preço tem o que esta janela nunca teve: a CONTA. Ela
// sabe que tela, tinta e frete são divididos pelo lote — então sabe
// dizer quanto o copo custa em 500 e em 1000, em vez de deixar o
// desconto no chute. A faixa nasce da mesma conta que faz o preço.
//
// O QUE FICA AQUI É A LEITURA. Quem abre o modelo continua vendo o que
// vale hoje para aquelas cores — some a edição, não a informação; e a
// única resposta para "onde mudo isso" é um link.
// ════════════════════════════════════════════════════════════
function PrecoPorQuantidade({ modelo }) {
  const cores = modelo.cores || [];
  const faixas = (cores.find(c => (c.price_tiers || []).length)?.price_tiers || [])
    .slice()
    .sort((a, b) => (Number(a.min_qty) || 0) - (Number(b.min_qty) || 0));
  const divergem = new Set(cores.map(c => JSON.stringify(c.price_tiers || []))).size > 1;
  const bases = new Set(cores.map(c => Number(c.sale_price) || 0));
  const precoBase = bases.size === 1 ? [...bases][0] : null;
  const minimo = Math.max(...cores.map(c => Number(c.min_order_qty) || 0), 0);
  const categoria = modelo.category_id || cores[0]?.category_id || null;
    ...
    <div className="space-y-3">
      <div className="rounded-xl border border-gray-200 p-3">
        <p className="text-xs font-semibold text-gray-700 mb-2">
          O que vale hoje para as {cores.length} cores
        </p>

        {divergem && (
          <p className="text-[11px] text-amber-600 mb-2">
            As cores têm faixas diferentes entre si. Abaixo está a da primeira que tem —
            aplicar pela Formação de Preço iguala todas.
          </p>
        )}

        <ul className="text-xs text-gray-600 space-y-1">
          <li>
            Preço de tabela — {precoBase != null ? <b>{brl(precoBase)}</b> : 'varia entre as cores'} cada
          </li>
          {minimo > 0 && <li>Mínimo do pedido — <b>{minimo}</b> un</li>}
          {!faixas.length ? (
            <li className="text-gray-500">Sem faixa: todo pedido sai pelo preço de tabela.</li>
          ) : faixas.map((t, i) => (
            <li key={i}>
              {t.max_qty == null
      ...
      <p className="flex items-start gap-1.5 text-[11.5px] text-gray-500">
        ...
        <span>
          O preço de tabela e o desconto por quantidade são a mesma conta, e moram na{' '}
          <Link to={categoria ? `/pricing/formacao?categoria=${categoria}` : '/pricing/formacao'}
            className="text-primary-600 hover:underline">Formação de Preço</Link>
          {' '}— lá a faixa sai da própria conta do lote, e vale para a categoria inteira.
        </span>
      </p>
    </div>
  );
  ...
  return (
    <Modal isOpen onClose={onClose} size="xl" title={modelo.titulo}
      ...
      <div className="space-y-4">
        ...
        {aba === 'preco' ? (
          <PrecoPorQuantidade modelo={modelo} />
        ) : aba === 'catalogo' ? (
          <CatalogoDaCategoria
