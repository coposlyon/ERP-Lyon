  ...
  async function submit(e) {
    ...
    try {
      const res = await storeApi.post('/quote', {
        customer: form,
        notes: form.notes,
        items: items.map(i => ({ product_id: i.product_id, product_name: i.product_name, color: i.color, quantity: i.quantity })),
      });
      setDone(res);
      clear();
    } catch (err) {
          ...
          {items.map(i => {
            ...
            return (
              <div key={k} className="bg-white rounded-2xl border border-gray-100 p-3 flex items-center gap-4">
                <div className="bg-gray-50 rounded-xl flex items-center justify-center w-20 h-24 flex-shrink-0">
                  <Bottle color={color} size={56} />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 mt-0.5">{fmt(i.unit_price)} / un</p>
                </div>
                ...
                <div className="text-right w-24">
                  <p className="font-bold text-gray-900">{fmt(i.unit_price * i.quantity)}</p>
                </div>
