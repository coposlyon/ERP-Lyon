...
test('configuração — padrões da Lyon quando nada foi salvo', () => {
  assert.deepStrictEqual(L.configLogistica({}), { acrescimo_pct: 12, ocupacao_limite_pct: 70, cobrar_caixa: true, unidades_padrao: 100 });
  assert.deepStrictEqual(L.configLogistica({ logistica: { acrescimo_pct: '0', cobrar_caixa: false } }), { acrescimo_pct: 0, ocupacao_limite_pct: 70, cobrar_caixa: false, unidades_padrao: 100 });
});

test('unidades padrão — regra sem unidades usa o padrão (100) e avisa', () => {
  const e = L.calcularEnvio([{ nome: 'Twister 550 ml', quantidade: 250, regra: { ...REGRA_LD, unidades_por_caixa: null, caixa_pequena_id: null } }], CAIXAS);
  assert.strictEqual(e.ok, true);
  assert.strictEqual(e.volumes, 3);                 // 250 ÷ 100, para cima
